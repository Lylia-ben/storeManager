...
const SquareProductTable: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await window.electronAPI.fetchProductsByType("Square");
      ...
      <Table>
        <TableHead>
          <TableRow style={{ backgroundColor: "#1565c0" }}>
            ...
            <TableCell style={{ color: "#ffffff" }}>Side Length</TableCell>
        ...
        <TableBody>
          {products.map((product) => (
            <TableRow key={product.id}>
              ...
              <TableCell>{product.sideLength}</TableCell>
...
export default SquareProductTable;

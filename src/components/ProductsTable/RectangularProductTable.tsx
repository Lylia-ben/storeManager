...
const RectangularProductTable: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const data = await window.electronAPI.fetchProductsByType("Rectangular");
      ...
      <Table>
        <TableHead>
          <TableRow style={{ backgroundColor: "#1565c0" }}>
            ...
            <TableCell style={{ color: "#ffffff" }}>Width</TableCell>
            <TableCell style={{ color: "#ffffff" }}>Height</TableCell>
...
export default RectangularProductTable;
